...
let $avg := sum(doc("thermostat_original.xml")//p:thermostatData/p:dailyMesurement[p:date = $x]/p:measurement/p:humidity) div count(doc("thermostat_original.xml")//p:thermostatData/p:dailyMesurement[p:date = $x])
order by $x
return
(<p:date>{data($x)}</p:date>,
<p:avg-humidity>{data($avg)}</p:avg-humidity>)
}</p:thermostatData>:)
